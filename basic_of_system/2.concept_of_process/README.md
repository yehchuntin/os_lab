...
1. **第一個參數（path/file）**：告訴系統**要執行哪個程式**
2. **第二個參數（arg/argv[0]）**：告訴**被執行的程式**它自己叫什麼名字
...
**原因**：
- **第一個參數**：系統需要知道程式檔案在哪裡
- **第二個參數**：程式需要知道它是被怎麼呼叫的（影響行為）

#### 實際案例：為什麼這很重要？

```c
// 案例 1：busybox 工具
// busybox 是一個包含多個工具的單一執行檔
execl("/bin/busybox", "ls", "-l", NULL);    // 表現得像 ls
execl("/bin/busybox", "grep", "text", NULL); // 表現得像 grep

// 案例 2：gzip/gunzip
// 同一個程式，根據被呼叫的名字決定壓縮還是解壓
execl("/bin/gzip", "gzip", "file.txt", NULL);   // 壓縮
execl("/bin/gzip", "gunzip", "file.gz", NULL);  // 解壓

// 案例 3：除錯工具
execl("/usr/bin/python", "my_script", "arg1", NULL);
// Python 解釋器會在錯誤訊息中顯示 "my_script"
```
...
```
呼叫 execl("/bin/cat", "mycat", "file.txt", NULL)
...
🧠 比喻：就像父母等小孩下課回家。
如果父母（父進程）不去「接小孩（回收資源）」——學校（作業系統）會一直留著那個小孩的資料（殭屍進程）。
